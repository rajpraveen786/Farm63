...
<div class="container mt-3">
    <nav aria-label="breadcrumb">
        ...
        <ol class="breadcrumb" style="background-color: transparent">
            ...
            <li class="breadcrumb-item"><a href="/admin/pages/{{ $pagename }}">{{ $pagename }} </a></li>
    ...
    <div class="card shadow-custom" style="border-radius: 8px">
        <div class="card-body">
            <h3 class="mb-3 color-prim">Banner - Add</h3>
            <form action="/admin/pages/{{ $pagename }}/subbanner/add" method="POST" class="forms-" enctype="multipart/form-data"> @csrf
                <div class="form-row my-2">
                    ...
                    <div class="form-group mt-2 col-md-6">
                        <input type="text" class="input-hov form-control {{ $errors->has('desc')? 'is-invalid ':'' }}" autocomplete="off" name="desc" id="inputDescription4" placeholder="Enter Description" value="{{ old('desc')??'' }}">
                    ...
                    <div class="mt-2 col-12">
                        ...
                        :oldlinkt='{{ json_encode(old('linkt')??'') }}'
                        :oldoptsel='{{ json_encode(old('optsel')??'') }}'
                        :errorlink='{{ json_encode($errors->first('linkt')??'') }}'
                        :erroroptsel='{{ json_encode($errors->first('optsel')??'') }}'></adminbannerindex>
