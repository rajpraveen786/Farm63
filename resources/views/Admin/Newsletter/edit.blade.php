...
<div class="container mt-3">
    ...
    <div class="card shadow-custom" style="border-radius: 8px">
        <div class="card-body">
            <h3 class="mb-3 color-prim">Newsletter - Edit</h3>
            <form action="/admin/newsletter/edit/{{ encrypt($data->id) }}" method="POST" class="forms-" enctype="multipart/form-data"> @csrf
                <div class="form-row my-2">
                    <div class="form-group mt-2 col-md-6">
                        <input type="text" class="input-hov form-control {{ $errors->has('title')? 'is-invalid ':'' }}" autocomplete="off" name="title" id="inputTitle4" placeholder="Enter Title" value="{{ old('title')??$data->title??'' }}">
                    ...
                    <adminnewsletterindex
                        :oldsubject="{{ json_encode(old('oldsubject')??$data->subject) }}"
                        :oldcontent="{{ json_encode(old('oldcontent')??$data->content) }}"
                        :olddatetime="{{ json_encode(old('olddatetime')??$data->datetime) }}"
                        :oldtemplate="{{ json_encode(old('oldtemplate')??$data->template) }}"
